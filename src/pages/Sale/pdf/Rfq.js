...
import { Formate } from "meta/Reusable/CalcData";
import moment from "moment";

const ReqForQuote = ({ data, handleComplete, title, hidden = false }) => {
  const [pdfSrc, setPdfSrc] = useState("");
  const [modalVisible, setModalVisible] = useState(false);
  // Add a table to the PDF
  const docName = title;
  const doc = new jsPDF();
  const netSubTotal = 12000;
  const jsPdfData = data?.products?.map((p, i) => [
    (i + 1).toString(),
    p.name,
    p.qty == 1 ? "No." : "Nos.",
    p.qty?.toString(),
  ...
  const address = `Project: ${data?.projectName || "***"}`;
  const customerName = `M/s. ${data?.vendor?.title}`;
  const docId = data.cuId;
  const generateInvoice = () => {
    ...
    doc.text("Remarks:", leftSideSet, tableHeight + 5);
    doc.setFont("helvetica", "normal");
    doc.text(
      "The information is confidential and intended solely for the use of the recipient(s) named above.",
      leftSideSet + 15,
      tableHeight + 5
    );
  ...
  return (
    <>
      <Tooltip color={"cyan"} title="Print/Download RFQ">
        <Button
          hidden={hidden}
          icon={<PrinterOutlined />}
          onClick={() => {
            generateInvoice();
            handleComplete(data?.rfqStatus);
          }}
        />
      </Tooltip>
      <Modal
