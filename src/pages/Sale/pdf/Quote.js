...
const Quote = ({ quotation, handleComplete, title, disabled = false }) => {
  ...
  const jsPdfData = quotation?.products?.map((p, i) => [
    (i + 1).toString(),
    p.name,
    p.qty == 1 ? "No." : "Nos.",
    p.qty?.toString(),
    Formate((12345).toString()),
    Formate((12345).toString() * p.qty),
  ]);
  ...
  const address = `Project: ${quotation?.projectName || "***"}`;
  const customerName = `M/s. ${quotation?.customer?.title}`;
  const docId = quotation.cuId;
  ...
  return (
    <>
      <Button
        ...
        onClick={() => {
          generateInvoice();
          handleComplete(quotation?.quotationStatus);
        }}
      >
        {title}
      </Button>
