...
const AccessoriesTable = ({
  ...
  useEffect(() => {
    setProducts(panelDetail?.accessories);
  }, [panels]);
      ...
      ...prevPanels.map((panel) => {
        if (panel._id === panelDetail._id) {
          const updatedPanel = {
            ...panel,
            save: (panel.save || 0) + 1,
            aCost: productCost([...panel.accessories, newData], estimation),
            accessories: [...panel.accessories, newData],
          };

          const panelCost = PanelSum(updatedPanel);
          console.log(updatedPanel, panelCost);
          return {
            ...updatedPanel,
            panelCost,
          };
      ...
      ...prevPanels.map((panel) => {
        if (panel._id === panelDetail._id) {
          const updatedPanel = {
            ...panel,
            save: (panel.save || 0) + 1,
            aCost: productCost(newData, estimation),
            accessories: newData,
          };
          const panelCost = PanelSum(updatedPanel);
          return {
      ...
      ...prevPanels.map((panel) => {
        if (panel._id === panelDetail._id) {
          const updatedPanel = {
            ...panel,
            save: (panel.save || 0) + 1,
            aCost: productCost(filterData, estimation),
            accessories: filterData,
          };
          const panelCost = PanelSum(updatedPanel);
          return {
            ...updatedPanel,
  ...
  const onPaste = (index, record) => {
    ...
    newPanels[panelIndex].accessories.splice(index + 1, 0, ...newSelected);
  ...
  const onCut = async (index, record) => {
    ...
    newPanels[panelIndex].accessories = newPanels[
      panelIndex
    ].accessories.filter((p) =>
      newProducts.some((newProduct) => newProduct.key === p.key)
    );
    newPanels[panelIndex].accessories.splice(index + 1, 0, ...newSelected);
      ...
      ...prevPanels.map((panel) => {
        if (panel._id === panelDetail._id) {
          const updatedPanel = {
            ...newPanels,
            save: (panel.save || 0) + 1,
            aCost: productCost(products, estimation),
          };
          const panelCost = PanelSum(updatedPanel);
          return {
            ...updatedPanel,
      ...
      ...prevPanels.map((panel) => {
        if (panel._id === panelDetail._id) {
          const updatedPanel = {
            ...panel,
            save: (panel.save || 0) + 1,
            aCost: productCost([...panel.accessories, ...newFile], estimation),
            accessories: [...panel.accessories, ...newFile],
          };
          const panelCost = PanelSum(updatedPanel);
          return {
            ...updatedPanel,
