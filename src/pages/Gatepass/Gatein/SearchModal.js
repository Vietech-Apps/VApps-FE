...
const SearchModal = ({ dataSources, setDataSources, open, setOpen }) => {
  ...
  const handleSelect = async (value, options) => {
    const fData = options?.find((p) => p._id === value);
    setDataSource([]);

    let newSelectedOption = fData?.products
      .filter(
        (selectedOption) =>
          !dataSources.some(
            (d) => d.po === value && d.productId === selectedOption.productId
          )
      )
      .map((selectedOption) => ({
        key: uuidv4(),
    ...
    if (newSelectedOption.length === 0) {
      message.info("All products already exist for this P.O in this GateIn");
    } else {
  ...
  const handlePush = (e, onCleanSelected) => {
    const dnew = e.map((d) => ({ ...d, deliveredQty: d.coming, check: true }));
