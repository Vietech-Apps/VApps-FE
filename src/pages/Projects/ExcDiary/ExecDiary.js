...
const Main = () => {
  ...
  const titleForm = `Exective Diary`;
  const listPath = `scrum/diary/list`;
  const createPath = `scrum/diary/create`;
  const updatePath = `scrum/diary/update`;
  ...
  const getList2 = async () => {
    try {
      const response = await jwtAxios.get(`ticket/listmanhour`);
