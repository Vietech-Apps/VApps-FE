...
import {updateCardById} from 'redux/actions/Projects/cardActions';
import {
  createNewList,
  updateListById,
} from 'redux/actions/Projects/listActions';
import InputCard from './InputCard';
import {
  createNewActivity,
  deleteActivityById,
} from 'redux/actions/Projects/activityActions';
...
export default function Board() {
  ...
  useEffect(() => {
    if (!_.isEmpty(currBoard)) {
      ...
      document.title = `${currBoard.name} | VieTech`;
    }
  }, [currBoard]);
  ...
  const onDragEnd = (result) => {
    ...
    const text = `${user.name} moved ${initialData.tasks[draggableId].name} from ${startList.name} to ${endList.name}`;
    const recentActivity = activities[activities.length - 1];
    if (
      recentActivity.text ===
        `${user.name} moved ${initialData.tasks[draggableId].name} from ${endList.name} to ${startList.name}` &&
      moment(recentActivity.createdAt).fromNow().includes('second')
    ) {
      dispatch(deleteActivityById(recentActivity._id));
    } else dispatch(createNewActivity({text, boardId: currBoard._id}));
  ...
  const submitHandler = () => {
    ...
    dispatch(
      createNewActivity({
        text: `${user.name} added ${listTitle} to this board`,
        boardId: currBoard._id,
      }),
    );
    setListTitle('');
            ...
            <div className={classes.editable}>
              <InputBase
                ...
                onBlur={() => {
                  ...
                  dispatch(updateBoardById(id, {name: text}));
