...
const PDFView = ({
  ...
  const jsPdfData = dataSource?.map((p, i) => [
    (i + 1).toString(),
    selected?.po?.cuId,
    p.code,
    p.name,
    p.deliveredQty <= 1 ? "No." : "Nos.",
    p.deliveredQty?.toString(),
    p.defectedQty ? p.defectedQty?.toString() : "0",
    (p.deliveredQty - p.defectedQty).toString(),
  ...
  const generateInvoice = () => {
    ...
    doc.setFontSize(10);
    doc.setTextColor(0);
    doc.text(`GRN: ${selected?.cuId}`, leftSideSet, headerHeight + 5);
    doc.setFont("helvetica", "normal");
    // doc.text(selected?.createdAt, leftSideSet, headerHeight + 5);
    doc.setFont("helvetica", "bold");
