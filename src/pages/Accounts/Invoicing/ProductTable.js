...
import AdjustmentModal from "../CreditNote/AdjustmentModal";
            ...
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              <div>
                      ...
                      <div className="flex justify-between gap-2 text-right">
                        <div colSpan={2} style={{ textAlign: "right" }}>
                          <Popover
                            content={
                              <div>
                                <p>Payment Ref: {c.cuId}</p>

                                <p>JrnEntry {c.journalEntry?.cuId}</p>
                                <p>
                                  Date {moment(c.data).format("DD-MM-YYYY")}
                                </p>
                              </div>
                            }
                          ...
                          <b style={{ fontSize: "12px" }}>Paid on {c.cuId}:</b>
                        </div>
                        <div style={{ textAlign: "right" }}>
                          <Typography.Text
                            type="success"
                ...
                {selected?.unallocated?.length > 0 &&
                  selected?.unallocated
                    ?.filter((c) => c.status == "Unallocated")
                    .map((c, i) => (
                      <div key={i}>
                        <hr />
                        <div className="flex justify-between gap-2 text-right">
      ...
      {selected?.unallocated?.length > 0 &&
        ...
        open && (
          <Unallocated
            ...
            route={`${invoiceInfo?.dataRoute}/allocation/${selected?._id}`}
          />
        )}
      {selected?.creditNotes?.length > 0 &&
        selected?.status !== "In Payment" &&
        openCN && (
          <AdjustmentModal
            setOpen={setOpenCN}
            open={openCN}
            route={`${invoiceInfo?.dataRoute}/adjustCN/${selected?._id}`}
            selected={selected}
            dataSource={dataSource}
            setSelected={setSelected}
            allocateData={allocateData}
