...
const CollectionCreateForm = ({
  ...
  useEffect(() => {
    setAccountType(
      selected?.contactBankAccount
        ? selected?.contactBankAccount?.accountType
        : selected?.contact?.bankAccounts[0]?.accountType
    );
    form.setFieldsValue({
      reference: selected?.cuId,
      amountDue: difference,
      account:
        selected?.contactBankAccount?._id ||
        selected?.contact?.bankAccounts[0]?._id,
    });
  }, [selected, difference]);
  const handleSelect = (e) => {
    let f = selected?.contact?.bankAccounts.find((c) => c._id == e);
    ...
    >
      ...
      >
        {/* <SelectSearch
          ...
          label={"Account"}
          name={`account`}
          where={{ _id: selected?.contactBankAccount }}
          columns={[
            {
              label: "Bank",
              size: 2,
        ...
        /> */}
        <Form.Item name="account" label="Bank Account">
          <Select
            allowClear
            onSelect={!selected?.contactBankAccount && handleSelect}
          >
            {selected?.contactBankAccount ? (
              <Select.Option value={selected?.contactBankAccount?._id}>
                {selected?.contactBankAccount?.accountType == "bank"
                  ? selected?.contactBankAccount?.accountNumber +
                    "-" +
                    selected?.contactBankAccount?.accountHolderName
                  : selected?.contactBankAccount?.accountHolderName}
              </Select.Option>
            ) : (
              selected?.contact?.bankAccounts?.map((c, i) => (
...
const RegisterPayment = ({
  ...
  const onCreate = async (values) => {
    ...
    const entries = [
      {
        account: `${selected?.contact?.accountPayable?.code}-${selected?.contact?.accountPayable?.name}`,
      ...
      {
        account: `${setting?.outstandingPaymentsAccount?.code}-${setting?.outstandingPaymentsAccount?.name}`,
        accountId: setting?.outstandingPaymentsAccount?._id,
        notes: "Vendor Payment",
        contact: selected?.contact?._id,
    ...
    try {
      const response = await jwtAxios.put(
        `accounts/Bill/regPayment/${selected?._id}`,
        values
      );
