...
const CreateForm = () => {
  ...
  useEffect(() => {
    ...
    setisChecked(selected?.internalTransfer || isChecked);
  ...
  const handleSelectContact = (e, option) => {
    let x = option?.find((c) => c._id == e);
    form.setFieldsValue({ contactAccount: x.bankAccounts[0]?._id });
    setContacts(x);
  };
          ...
          >
            <Row gutter={24}>
              <Col xs={24} sm={18}>
                ...
                {paymentType == "Send" && !isChecked && (
                  <Form.Item
                    name={"contactAccount"}
                    label="Vendor Bank Account"
                  >
                    <Select>
                      {contacts?.bankAccounts?.map((c) => (
                ...
                <SelectSearch
                  ...
                  name={"companyAccount"}
                  otherParams={
                    selected ? { _id: selected?.acountHolder?._id } : null
                  }
